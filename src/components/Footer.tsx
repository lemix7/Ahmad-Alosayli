...
const DEFAULT_LINKS: FooterLink[] = [
  {
    label: "LinkedIn",
    href: "https://www.linkedin.com/in/ahmad-sapil-576732380/?skipRedirect=true",
    external: true,
  },
  { label: "Github", href: "https://github.com/lemix7", external: true },
  {
    label: "My Photography",
    href: "https://aagallery.framer.website/?utm_source=ig&utm_medium=social&utm_content=link_in_bio",
    external: true,
  },
];
...
const ContactButton = ({ ContactEmail = EMAIL }) => {
  ...
  return (
    <StaggerButton
      as="button"
      label={copied ? "Copied" : "Get in touch"}
      onClick={handleClick}
      className="group inline-flex items-center justify-center px-4 py-2 sm:h-9 sm:px-6 sm:py-8 rounded-full border border-neutral-400/80 hover:border-white bg-transparent hover:bg-gradient-light text-gradient-light hover:text-black transition-all duration-300 shadow-sm active:scale-95"
      textClassName="text-sm sm:text-base md:text-3xl lg:text-4xl font-medium tracking-tight uppercase leading-none"
    />
  );
};
